...
func (s *eventService) GetCalendarForUser(ctx context.Context, userID uuid.UUID, start, end time.Time) ([]*e.Event, error) {
	...
	for _, id := range activeEnrollments {
		sectionIDMap[id] = true
	}
	for _, id := range staffSections {
		sectionIDMap[id] = true
	}
...
func (s *eventService) GetAnnouncements(ctx context.Context, orgID uuid.UUID, start, end time.Time, limit, offset int) ([]*e.Event, error) {
	if limit <= 0 {
		limit = 10
	}
...
func (s *eventService) GetEvents(ctx context.Context, orgID uuid.UUID, start, end time.Time, limit, offset int) ([]*e.Event, error) {
	if limit <= 0 {
		limit = 20
	}
	...
	filter := e.EventFilter{
		OrganizationID: orgID,
		Types:          []e.EventType{e.Vanilla, e.Meeting},
		IncludeGlobal:  true,
		StartTime:      start,
		EndTime:        end,
		Limit:          limit,
